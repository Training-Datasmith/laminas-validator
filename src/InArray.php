 ...
 * }
 */
final class In_Array extends Abstract_Validator
{
    ...
    protected array $message_templates = [self::NOT_IN_ARRAY => 'The input was not found in the haystack'];
    /**
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        if (self::COMPARE_NOT_STRICT_AND_PREVENT_STR_TO_INT_VULNERABILITY === $this->strict && (is_int($value) || is_float($value))) {
            $value = (string) $value;
        }
        $this->set_value($value);
        if ($this->recursive) {
            $iterator = new Recursive_Iterator_Iterator(new Recursive_Array_Iterator($haystack));
