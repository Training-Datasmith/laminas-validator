...
final class Business_Identifier_Code extends Abstract_Validator
{
    public const INVALID = 'valueNotBic';
    public const NOT_STRING = 'valueNotString';
    public const NOT_VALID_COUNTRY = 'valueNotCountry';
    /** @var array<string, string> */
    protected array $message_templates = [self::NOT_STRING => 'Invalid type given; string expected', self::INVALID => 'Invalid BIC format', self::NOT_VALID_COUNTRY => 'Invalid country code'];
    /**
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        if (empty($value) || !preg_match(self::REGEX_BIC, $value, $matches)) {
            $this->error(self::INVALID);
            return false;
        }
        if (!$this->is_swift_valid_country($matches['country'])) {
            $this->error(self::NOT_VALID_COUNTRY);
            return false;
        }
        return true;
    }
    private function is_swift_valid_country(string $country_code): bool
    {
        $country_code = strtoupper($country_code);
        return in_array($country_code, self::ISO_COUNTRIES, true) || $country_code === self::KOSOVO_EXCEPTION;
    }
}