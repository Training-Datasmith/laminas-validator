 ...
 * }
 */
final class Callback extends Abstract_Validator
{
     ...
     */
    protected array $message_templates = [self::INVALID_VALUE => 'The input is not valid', self::INVALID_CALLBACK => 'An exception has been raised within the callback'];
    /** @var Closure(mixed, array<string, mixed>, mixed...): bool */
    private readonly Closure $callback;
    private readonly bool $throw_exceptions;
    /** @var array<array-key, mixed> */
    private readonly array $callback_options;
    ...
    public function __construct(array|callable $options)
    {
        ...
        $callback_options = $options['callbackOptions'] ?? [];
        ...
        $callback = $bind ? $callback(...)->bind_to($this) : $callback(...);
        $this->callback = $callback;
        $this->throw_exceptions = $throw;
        $this->callback_options = $callback_options;
    ...
    public function is_valid(mixed $value, ?array $context = null): bool
    {
        ...
        try {
            $result = ($this->callback)($value, $context ?? [], ...$this->callback_options);
