 ...
 * }
 */
final class Number_Comparison extends Abstract_Validator
{
    ...
    protected array $message_templates = [self::ERROR_NOT_NUMERIC => 'Expected a numeric value', self::ERROR_NOT_GREATER_INCLUSIVE => 'Values must be greater than or equal to %min%. Received "%value%"', self::ERROR_NOT_GREATER => 'Values must be greater than %min%. Received "%value%"', self::ERROR_NOT_LESS_INCLUSIVE => 'Values must be less than or equal to %max%. Received "%value%"', self::ERROR_NOT_LESS => 'Values must be less than %max%. Received "%value%"'];
    /** @var array<string, string|array<string, string>> */
    protected array $message_variables = ['min' => 'min', 'max' => 'max'];
    /** @var numeric|null */
    ...
    public function __construct(array $options = [])
    {
        ...
        if (!is_numeric($min) && !is_numeric($max)) {
            throw new InvalidArgumentException('A numeric option value for either min, max or both must be provided');
        }
        if ($min !== null && $max !== null && $min > $max) {
            throw new InvalidArgumentException('The minimum constraint cannot be greater than the maximum constraint');
        }
        $this->min = $min;
        $this->max = $max;
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        if ($this->min !== null && $this->inclusive_min && $value < $this->min) {
        ...
        if ($this->max !== null && $this->inclusive_max && $value > $this->max) {
