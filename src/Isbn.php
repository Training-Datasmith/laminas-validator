 ...
 * }
 */
final class Isbn extends Abstract_Validator
{
     ...
     */
    protected array $message_templates = [self::INVALID => 'Invalid type given. String or integer expected', self::NO_ISBN => 'The input is not a valid ISBN number'];
     ...
     */
    private function detect_format(string $value): ?string
    {
        // check for ISBN-10
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        $type = $this->detect_format($value);
        if ($type === null) {
            $this->error(self::NO_ISBN);
            return false;
        }
        $checksum = $type === self::ISBN10 ? $this->calculate_isbn10checksum($value) : $this->calculate_isbn13checksum($value);
        // validate
    ...
    private function calculate_isbn10checksum(string $value): string
    {
        $sum = 0;
        for ($i = 0; $i < 9; $i++) {
    ...
    private function calculate_isbn13checksum(string $value): string
    {
        $sum = 0;
        for ($i = 0; $i < 12; $i++) {
            if ($i % 2 === 0) {
