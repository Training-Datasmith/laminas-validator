 ...
 * }
 */
final class Timezone extends Abstract_Validator
{
    ...
    public const LOCATION = 0b1;
    public const ABBREVIATION = 0b10;
    public const ALL = 0b11;
    /** @var array<string, string> */
    protected array $message_templates = [self::INVALID => 'Invalid timezone given.', self::INVALID_TIMEZONE_LOCATION => 'Invalid timezone location given.', self::INVALID_TIMEZONE_ABBREVIATION => 'Invalid timezone abbreviation given.'];
    ...
    public function __construct(array $options = [])
    {
        ...
        if (!is_int($type) || ($type & self::ALL) === 0) {
            throw new InvalidArgumentException('The type option must be an int-mask of the type constants');
        }
        $this->type = $type;
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        if ($this->type === self::ALL) {
            if (!array_key_exists(strtolower($value), DateTimeZone::list_abbreviations()) && !in_array($value, DateTimeZone::list_identifiers(), true)) {
                $this->error(self::INVALID);
                return false;
            }
        }
        if ($this->type === self::LOCATION && !in_array($value, DateTimeZone::list_identifiers(), true)) {
            $this->error(self::INVALID_TIMEZONE_LOCATION);
            return false;
        }
        if ($this->type === self::ABBREVIATION && !array_key_exists(strtolower($value), DateTimeZone::list_abbreviations())) {
