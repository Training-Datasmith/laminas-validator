 ...
 * }
 */
final class Hostname extends Abstract_Validator
{
    ...
    protected array $message_variables = ['tld' => 'tld'];
    ...
    public function __construct(array $options = [])
    {
        ...
        $this->ip_validator = $options['ipValidator'] ?? new Ip();
        $this->use_idn_check = $options['useIdnCheck'] ?? true;
        $this->use_tld_check = $options['useTldCheck'] ?? true;
    }
    /**
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        $allow_ip = ($this->allow & self::ALLOW_IP) === self::ALLOW_IP;
        $allow_local = ($this->allow & self::ALLOW_LOCAL) === self::ALLOW_LOCAL;
        $allow_dns = ($this->allow & self::ALLOW_DNS) === self::ALLOW_DNS;
        $allow_uri = ($this->allow & self::ALLOW_URI) === self::ALLOW_URI;
        $this->set_value($value);
        // Check input against IP address schema
        if ((preg_match('/^[0-9.]*$/', $value) && str_contains($value, '.') || preg_match('/^[0-9a-f:.]*$/i', $value) && str_contains($value, ':')) && $this->ip_validator->is_valid($value)) {
            if (!$allow_ip) {
        ...
        if (count($domain_parts) === 4 && preg_match('/^[0-9.a-e:.]*$/i', $value) && $this->ip_validator->is_valid($value)) {
            $this->error(self::INVALID_LOCAL_NAME);
        }
        /** @psalm-var non-empty-list<string> $domainParts */
        $utf8str_wrapper = String_Utils::get_wrapper('UTF-8');
        // Check input against DNS hostname schema
        if (count($domain_parts) > 1 && $utf8str_wrapper->strlen($value) >= 4 && $utf8str_wrapper->strlen($value) <= 254) {
            $status = false;
            do {
                // First check TLD
                $matches = [];
                $last = end($domain_parts);
                reset($domain_parts);
                assert(is_string($last));
                if (preg_match('/([^.]{2,63})$/u', $last, $matches) || array_key_exists($last, $this->valid_idns)) {
                    ...
                    if (str_starts_with($this->tld, 'xn--')) {
                        $this->tld = $this->decode_punycode(substr($this->tld, 4));
                        if ($this->tld === false) {
                            return false;
                        }
                    } else {
                    ...
                    $removed_tld = false;
                    if ($this->use_tld_check) {
                        if (!in_array(strtolower($this->tld), $this->valid_tlds) && !in_array($this->tld, $this->valid_tlds)) {
                    ...
                    if ($this->use_idn_check && isset($this->valid_idns[$this->tld])) {
                        if (is_string($this->valid_idns[$this->tld])) {
                            $file_path = __DIR__ . '/' . $this->valid_idns[$this->tld];
                    ...
                    $check = 0;
                    $last_domain_part = end($domain_parts);
                    if (!$removed_tld) {
                        $last_domain_part = prev($domain_parts);
                    }
                    foreach ($domain_parts as $domain_part) {
                        // Decode Punycode domain names to IDN
                        if (str_starts_with($domain_part, 'xn--')) {
                            $domain_part = $this->decode_punycode(substr($domain_part, 4));
                        ...
                        if ($utf8str_wrapper->strpos($domain_part, '-') === 0 || $utf8str_wrapper->strlen($domain_part) > 2 && $utf8str_wrapper->strpos($domain_part, '-', 2) === 2 && $utf8str_wrapper->strpos($domain_part, '-', 3) === 3 || $utf8str_wrapper->substr($domain_part, -1) === '-') {
                        ...
                        $checked = false;
                        $is_sub_domain = $domain_part !== $last_domain_part;
                        $part_regex_chars = $is_sub_domain ? ['/^[a-z0-9_\x2d]{1,63}$/i'] + $regex_chars : $regex_chars;
                        foreach ($part_regex_chars as $regex_key => $regex_char) {
                            $status = preg_match($regex_char, $domain_part);
                            if ($status > 0) {
                                $length = 63;
                                if (array_key_exists($this->tld, $this->idn_length) && array_key_exists($regex_key, $this->idn_length[$this->tld])) {
                                    $length = $this->idn_length[$this->tld];
                                }
                                if ($utf8str_wrapper->strlen($domain_part) > $length) {
                                    $this->error(self::INVALID_HOSTNAME);
                                    $status = false;
                                } else {
                                    $checked = true;
     ...
     */
    private function decode_punycode(string $encoded): string|false
    {
