 ...
 * }
 */
final class Is_Countable extends Abstract_Validator
{
     ...
     */
    protected array $message_templates = [self::NOT_COUNTABLE => 'The input must be an array or an instance of \Countable', self::NOT_EQUALS => "The input count must equal '%count%'", self::GREATER_THAN => "The input count must be less than '%max%', inclusively", self::LESS_THAN => "The input count must be greater than '%min%', inclusively"];
    /** @var array<string, string|array<string, string>> */
    protected array $message_variables = ['count' => 'count', 'min' => 'min', 'max' => 'max'];
    ...
    public function __construct(array $options = [])
    {
        ...
        if ($count !== null && ($min !== null || $max !== null)) {
            throw new InvalidArgumentException('The `count` option is mutually exclusive with the `min` and `max` options');
        }
        if ($max !== null && $min !== null && $max < $min) {
            throw new InvalidArgumentException('The `max` option cannot be less than the `min` option');
        }
        $this->min = $min;
        $this->max = $max;
        $this->count = $count;
