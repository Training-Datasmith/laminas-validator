 ...
 * }
 */
final class Ip extends Abstract_Validator
{
    public const INVALID = 'ipInvalid';
    public const NOT_IP_ADDRESS = 'notIpAddress';
    /** @var array<string, string> */
    protected array $message_templates = [self::INVALID => 'Invalid type given. String expected', self::NOT_IP_ADDRESS => 'The input does not appear to be a valid IP address'];
    private readonly bool $allowipv4;
    // Enable IPv4 Validation
    private readonly bool $allowipv6;
    // Enable IPv6 Validation
    private readonly bool $allowipvfuture;
    // Enable IPvFuture Validation
    private readonly bool $allowliteral;
    ...
    public function __construct(array $options = [])
    {
        ...
        if ($this->allowipv4 === false && $this->allowipv6 === false && $this->allowipvfuture === false) {
    ...
    public function is_valid(mixed $value): bool
    {
        ...
        if ($this->allowipv4 && $this->validate_i_pv4($value)) {
            return true;
        }
        ...
        if ($this->allowipv6 && $this->validate_i_pv6($value) || $this->allowipvfuture && $this->validate_i_pv_future($value)) {
            return true;
        }
     ...
     */
    private function validate_i_pv4(string $value): bool
    {
        if (preg_match('/^([01]{8}\.){3}[01]{8}\z/i', $value)) {
            // binary format  00000000.00000000.00000000.00000000
            $value = sprintf('%s.%s.%s.%s', bindec(substr($value, 0, 8)), bindec(substr($value, 9, 8)), bindec(substr($value, 18, 8)), bindec(substr($value, 27, 8)));
        } elseif (preg_match('/^([0-9]{3}\.){3}[0-9]{3}\z/i', $value)) {
            // octet format 777.777.777.777
            $value = sprintf('%d.%d.%d.%d', substr($value, 0, 3), substr($value, 4, 3), substr($value, 8, 3), substr($value, 12, 3));
        } elseif (preg_match('/^([0-9a-f]{2}\.){3}[0-9a-f]{2}\z/i', $value)) {
            // hex format ff.ff.ff.ff
            $value = sprintf('%s.%s.%s.%s', hexdec(substr($value, 0, 2)), hexdec(substr($value, 3, 2)), hexdec(substr($value, 6, 2)), hexdec(substr($value, 9, 2)));
        }
     ...
     */
    private function validate_i_pv6(string $value): bool
    {
        if (strlen($value) < 3) {
            return $value === '::';
        }
        if (str_contains($value, '.')) {
            ...
            if (!($lastcolon !== false && $this->validate_i_pv4(substr($value, $lastcolon + 1)))) {
                return false;
            }
     ...
     */
    private function validate_i_pv_future(string $value): bool
    {
        /*
         * ABNF:
